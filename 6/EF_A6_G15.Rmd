...
simpleVolatilityApproach <- function(input_stocksList,
                                     input_window,
                                     input_returns,
                                     input_gamma){
  # Create a new data frame with the specified dates and empty weight columns
  ...
  # Obtain portfolio weights using a simple mean and volatility approach
  for (i in start_index:end_index) {
    # Ensure data_subset is numeric
    ...
    result_simple[i-W,2:(K+1)] <- outcome$solution
  }
...
movingAverageApproach <- function(input_stocksList,
                                  input_window,
                                  input_returns,
                                  input_gamma){
  # Create a new data frame with the specified dates and empty weight columns
...
SV_portfolio <- simpleVolatilityApproach(input_stocksList = stocks_list,
                             input_window = W,
                             input_returns = separate_returns,
                             input_gamma = gamma)
MA_portfolio <- movingAverageApproach(input_stocksList = stocks_list,
                             input_window = W,
                             input_returns = separate_returns,
                             input_gamma = gamma)

simpleVolatility <- 
  ggplot(SV_portfolio$singleTable, 
...
#Export the plot
ggsave('figures/a_SV_optimizationPlot.png', plot = simpleVolatility,
       width = 10, height = 6, dpi = 300)
print(simpleVolatility)

movingAverage <- 
  ggplot(MA_portfolio$singleTable, 
         aes(x = date, y = Weight, color = Stock, linetype = Stock)) +
  geom_line(,linewidth=1) +  # Draw lines for each stock
  labs(x = "Date",
       y = "Weight") +
  ...
  theme(legend.title = element_blank())  # Remove legend title
#Export the plot
ggsave('figures/a_MA_optimizationPlot.png', plot = movingAverage,
       width = 10, height = 6, dpi = 300)
print(movingAverage)
```
...
# Plot the realized 
realized <- ggplot(portfolio_returns_SV, 
  ...
  theme(legend.title = element_blank())  # Remove legend title
# Export the plot
ggsave('figures/b_realized_plot.png', plot = realized,
       width = 10, height = 6, dpi = 300)
print(realized)
...
computeSharpRatio(portfolio_returns_SV)

computeSharpRatio(portfolio_returns_MA)
```
# Question 2
