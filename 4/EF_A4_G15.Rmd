...
returns_df <- data.frame(date = date_lastFiveYear[2:1259],
                         simpleReturns_forecast = forecast_returns,
...
ggplot(returns_df, aes(x = date)) +
  geom_line(aes(y = simpleReturns_realized, color = 'Realized Returns'),
            linewidth = 1) +
  geom_line(aes(y = simpleReturns_forecast, color = 'Forecast Returns'),
            linewidth = 1) +
  labs(title = 'Forecast vs Realized for HSBC Daily Simple Returns (2015M11-2020M11)',
    x = 'Date', y = 'Simple Returns', color = 'Series') +
  scale_color_manual(values = c('Realized Returns' = 'blue',
                                'Forecast Returns' = 'red')) +
  theme_minimal() +
